...
import net.cyndeline.rlgraph.drawings.planar.orthogonal.constraints.improvementHeuristic.VisibilityGraphFactory
import net.cyndeline.rlgraph.drawings.planar.orthogonal.constraints.{SegmentFactory, ShapeGraphFactory}
import net.cyndeline.rlgraph.drawings.planar.orthogonal.drawing.{DrawingFactory, OrthogonalLayout}
import net.cyndeline.rlgraph.drawings.planar.orthogonal.network.DualMinimumCostFlow.DualMinimumCostFlow
import net.cyndeline.rlgraph.drawings.planar.orthogonal.operation.OrthoRepresentationComputation
import net.cyndeline.rlgraph.drawings.planar.orthogonal.representation._
...
class OrthogonalLayoutAlgorithm[VType : TypeTag : ClassTag, EType[X] <: UnDiEdge[X]: ({type l[M[_]] = TypeTag[M[VType]]})#l : ({type l[M[_]] = ClassTag[M[VType]]})#l] extends Orthogonalizer[VType, EType] {
  ...
  def orthogonalize(graph: Graph[VType, EType]): OrthogonalLayout[VType, EType] = {
    require(!multiEdgesExists(graph), "The algorithm currently doesn't support graphs with more than one edge " +
      "connecting a pair of vertices. Merge any such edge pair into a single edge and track it externally.")
    if (graph.isEmpty)
    ...
    val horizontalAssignment: Set[Segment[VType]] = if (shapeGraphs._1.nonEmpty) {
      dualMinimumCostFlow.assignSegmentCoordinates(shapeGraphs._1)
    } else {
      Set()
    }
    val verticalAssignment: Set[Segment[VType]] = if (shapeGraphs._2.nonEmpty) {
    ...
    do {
      ...
      finalHorizontalSegments = if (horizontalVisibilityGraph.nonEmpty) {
      ...
      finalVerticalSegments = if (verticalVisibilityGraph.nonEmpty) {
  ...
  private def multiEdgesExists(graph: Graph[VType, EType]): Boolean = {
    graph.nodes.exists(n => n.neighbors.exists(neighbor => neighbor.connectionsWith(n).size > 1))
  }
}
