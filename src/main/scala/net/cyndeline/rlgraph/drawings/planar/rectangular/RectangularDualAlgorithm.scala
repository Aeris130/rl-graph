...
import net.cyndeline.rlgraph.drawings.planar.rectangular.help._
...
/**
 * Constructs algorithms used to compute rectangular duals. For a description of the algorithm, see the
...
class RectangularDualAlgorithm[VType : ClassTag : TypeTag, EType[X] <: UnDiEdge[X]: ({type l[M[_]] = TypeTag[M[VType]]})#l] private
  ...
  private def executeRegularAlgorithm(graph: Graph[VType, EType]): RectangularLayout[VType, EType] = {
    require(graph.nonEmpty, "Cannot compute a rectangular dual from an empty graph.")
