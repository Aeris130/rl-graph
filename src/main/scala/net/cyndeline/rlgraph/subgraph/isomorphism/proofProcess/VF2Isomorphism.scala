 ...
 *
 * Note: This class was written by Andrius Velykis, and is available as part of the ProofProcess repo at
 * https://github.com/andriusvelykis/proofprocess/tree/master/lib/graph-isomorphism.
 *
 * The reason for including this source in the rlGraph project is to keep it up to date with the current version
 * of scalas graph library, as the original repo library compiles against 2.10.4, which has since had some of its
 * features deprecated.
 *
...
trait VF2Isomorphism[N1, E1[X1] <: EdgeLikeIn[X1], N2, E2[X2] <: EdgeLikeIn[X2]] {
  ...
  class State(val mapping: Map[Node2, Node1], val ord: Ordering[_ >: Node2]) {
    ...
    def terminals[Node](mapped: Set[Node], direction: Node => Set[Node]): Set[Node] = {
      val linked = mapped flatMap direction
  ...
  def apply[N1, E1[X1] <: EdgeLikeIn[X1], N2, E2[X2] <: EdgeLikeIn[X2]](
    ...
    new VF2Isomorphism[N1, E1, N2, E2] {
      ...
      override val isFeasibleNode: (Node1, Node2) => Boolean = matchNode match {
        case Some(matcher) => (n, m) => matcher(n, m)
        case None => (_, _) => true
      }
